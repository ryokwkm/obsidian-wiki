...
- **When a candidate's `sources:` points into `_source_docs/`** and the question needs what distillation drops — an exact quote, a figure, the bibliography — `Read` that file. It is the vault's verbatim copy of the original and is kept out of the search index on purpose, so this hop is the only way to reach it. Where the two disagree, prefer the distilled page: the distillation may have corrected the original.

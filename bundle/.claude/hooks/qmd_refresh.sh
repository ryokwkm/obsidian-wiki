...
#
# 前半（ラッパーの置き場を前置できるようにする）の理由: 同梱ランチャー `~/.bun/bin/qmd` は
# node を優先して起動するが、`bun install -g` では better-sqlite3 がビルドされず node 経路は
# 落ちる。bun 実行を強制するラッパーを使う環境では、その置き場を `QMD_BIN_DIR` で前置する
# （先頭の `~` は hook が展開する —— ホームが違う複数マシンへ同じ設定を配れるようにするため）。
# PATH の並び次第で壊れた方を掴むので、確実な方を前置する。hook は非対話・非ログインシェルで
# 走るため `.zshrc` の PATH が無いことも前提に置く。
#
